...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
守护进程顾名思义此进程的作用是在守护 Binder 服务的一个进程，往往守护进程都是在设备启动的时候开始执行，直至设备关机。Binder 的守护进程名为 ServiceManager 进程，在 Linux 的启动配置项中有一项就是启动 ServiceManager 进程，ServiceManager 自动之后首先会进入main 函数
```c
